...
print "label,request_type,samples,response_time,std,ci95,database_time,std,ci95,acquire_conn_time,std,ci95,serialization_time,std,ci95,deserialization_time,std,ci95,release_conn_time,std,ci95,statement_exec_time,std,ci95"
for request_type in range(8):
    if request_type == 5 or request_type == 6: continue # skip login and logout messages

    sys.stdout.write(os.path.basename(filename)[filenameStart:filenameEnd] + ',' + str(request_type) + ',' + str(len(response_time[request_type])) + ',')

    response_time_array         = np.array(response_time          [request_type])
    database_time_array         = np.array(database_time          [request_type])
    acquire_conn_time_array     = np.array(acquire_conn_time      [request_type])
    serialization_time_array    = np.array(serialization_time     [request_type])
    deserialization_time_array  = np.array(deserialization_time   [request_type])
